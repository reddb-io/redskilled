...
async function gatedOffDir(): Promise<string> {
  ...
  // the per-directory gate (ADR 0067) treats brain as disabled.
  const root = await mkdtemp(join(tmpdir(), "brain-inert-mcp-"));
  roots.push(root);
  return root;
}
...
describe("gated-off mcp path (issue #843)", () => {
  test(
    ...
    async () => {
      ...
      expect(init.result.serverInfo.name).toBe("red-brain");
