...
1. **Mount the parent `.git` dir at a deterministic POSIX path** —
   `/.red-castle-parent-git`. This gives the parent git directory a stable,
...
2. **Create a corrected `.git` file** with the `gitdir:` path rewritten to
   ...
   `gitdir: /.red-castle-parent-git/worktrees/abc`. Mount this file at
...
5. Remaps the parent `.git` dir mount to `/.red-castle-parent-git`.
...
- A small temp file is created per sandbox session. It is cleaned up when the
  ...
  normal temp cleanup.
- The `/.red-castle-parent-git` path is reserved inside the sandbox. This is
