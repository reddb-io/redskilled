...
describe("checkDockerHealth", () => {
  it("returns ok when docker info succeeds", async () => {
    ...
    await expect(Effect.runPromise(checkDockerHealth)).resolves.toBe("ok");
  });

  it("returns not-installed when docker is missing", async () => {
    mockExecFile.mockImplementation((_cmd, _args, _opts, cb: any) => {
    ...
    await expect(Effect.runPromise(checkDockerHealth)).resolves.toBe(
      "not-installed",
    );
  });

  it("returns not-running when docker info fails for another reason", async () => {
    mockExecFile.mockImplementation((_cmd, _args, _opts, cb: any) => {
      cb(new Error("daemon unavailable"), "", "");
      return undefined as any;
    });
