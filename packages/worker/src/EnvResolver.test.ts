...
describe("resolveEnv", () => {
  it("returns all key-value pairs from .red-castle/.env", async () => {
    const dir = await makeDir();
    await mkdir(join(dir, ".red-castle"));
    await writeFile(
      join(dir, ".red-castle", ".env"),
  ...
  it("root .env is ignored even when .red-castle/.env also exists", async () => {
    const dir = await makeDir();
    ...
    expect(env["SHARED"]).toBe("sc"); // only .red-castle/.env is used
  });

  it("falls back to process.env for keys declared in .red-castle/.env", async () => {
    const dir = await makeDir();
    await mkdir(join(dir, ".red-castle"));
    // .red-castle/.env declares the key but with empty value
    await writeFile(join(dir, ".red-castle", ".env"), "MY_TOKEN=\n");
  ...
  it("does NOT pull keys from process.env that are not in .red-castle/.env", async () => {
    const dir = await makeDir();
    await mkdir(join(dir, ".red-castle"));
    await writeFile(join(dir, ".red-castle", ".env"), "DECLARED_KEY=value\n");
  ...
  it(".red-castle/.env takes precedence over process.env", async () => {
    const dir = await makeDir();
    await mkdir(join(dir, ".red-castle"));
    await writeFile(join(dir, ".red-castle", ".env"), "MY_VAR=sc-val\n");
  ...
  it("ignores comments and blank lines in .red-castle/.env", async () => {
    const dir = await makeDir();
    await mkdir(join(dir, ".red-castle"));
    await writeFile(
      join(dir, ".red-castle", ".env"),
  ...
  it("does no validation — returns whatever keys are present in .red-castle/.env", async () => {
    const dir = await makeDir();
    await mkdir(join(dir, ".red-castle"));
  ...
  it("strips matching single quotes from values", async () => {
    const dir = await makeDir();
    await mkdir(join(dir, ".red-castle"));
    await writeFile(join(dir, ".red-castle", ".env"), "TOKEN='my-token'\n");
  ...
  it("leaves mismatched quotes as-is", async () => {
    const dir = await makeDir();
    await mkdir(join(dir, ".red-castle"));
    await writeFile(join(dir, ".red-castle", ".env"), `KEY="value'\n`);
  ...
  it("leaves interior quotes as-is", async () => {
    const dir = await makeDir();
    await mkdir(join(dir, ".red-castle"));
    await writeFile(join(dir, ".red-castle", ".env"), 'KEY=some"thing\n');
  ...
  it("handles empty quoted values", async () => {
    const dir = await makeDir();
    await mkdir(join(dir, ".red-castle"));
    await writeFile(join(dir, ".red-castle", ".env"), 'KEY=""\n');

    const env = await runResolveEnv(dir);
    expect(env).toEqual({});
  });

  it("process.env fallback works for keys in .red-castle/.env too", async () => {
    const dir = await makeDir();
    await mkdir(join(dir, ".red-castle"));
    await writeFile(join(dir, ".red-castle", ".env"), "FALLBACK_KEY=\n");
  ...
  it("unescapes \\n in double-quoted values", async () => {
    const dir = await makeDir();
    await mkdir(join(dir, ".red-castle"));
    await writeFile(join(dir, ".red-castle", ".env"), 'KEY="line1\\nline2"\n');
  ...
  it("preserves internal whitespace in double-quoted values", async () => {
    const dir = await makeDir();
    await mkdir(join(dir, ".red-castle"));
    await writeFile(join(dir, ".red-castle", ".env"), 'KEY="  spaced  "\n');
  ...
  it("handles escaped backslash before n in double-quoted values", async () => {
    const dir = await makeDir();
    await mkdir(join(dir, ".red-castle"));
    await writeFile(join(dir, ".red-castle", ".env"), 'KEY="a\\\\nb"\n');
  ...
  it("parses unquoted values unchanged", async () => {
    const dir = await makeDir();
    await mkdir(join(dir, ".red-castle"));
    await writeFile(join(dir, ".red-castle", ".env"), "KEY=plain\n");
