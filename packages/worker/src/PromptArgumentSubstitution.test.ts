...
describe("PromptArgumentSubstitution", () => {
  ...
  it("does not mark inline code spans that merely end in '!' (e.g. Rust macros)", async () => {
    const { layer } = setup();
    // A '!' that is the tail of a markdown code span must NOT be treated as a
    // shell block: otherwise `!` + the span's closing backtick false-matches
    // and the preprocessor shell-execs the prose between code spans (#756).
    const prompt =
      "Use `assert!` by default; `debug_assert!` only in measured hot loops.";
    const result = await run(prompt, {}, layer);
    expect(result).toBe(prompt);
    expect(result).not.toContain("\x01");
  });

  it("still marks a real shell block adjacent to a macro code span", async () => {
    const { layer } = setup();
    // `vec!` is a code span (not a shell block); the space-preceded
    // `` !`echo hi` `` is a real shell block and must still be marked.
    const result = await run("Prefer `vec!`. Then !`echo hi`", {}, layer);
    expect(result).toBe("Prefer `vec!`. Then !\x01`echo hi`");
  });
