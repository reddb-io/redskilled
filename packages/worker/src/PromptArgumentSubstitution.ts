...
// Matches a deliberate `` !`command` `` shell block. The negative lookbehind
// forbids a word char or backtick immediately before the `!`, so a markdown
// inline code span that merely *ends* in `!` (e.g. Rust macros `` `assert!` ``,
// `` `debug_assert!` ``) is NOT mistaken for a shell block — otherwise its `!`
// would pair with the span's closing backtick (`` !` ``) and the preprocessor
// would shell-exec the prose up to the next backtick (red-skills #756). A real
// shell block is preceded by whitespace / start / a delimiter, which all pass.
const SHELL_BLOCK_PATTERN = /(?<![A-Za-z0-9_`])!`([^`]+)`/g;
