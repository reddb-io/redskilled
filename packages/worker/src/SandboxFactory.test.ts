...
/** Find the sole worktree directory created under hostDir/.red-castle/worktrees. */
...
describe("WorktreeDockerSandboxFactory — isolated providers", () => {
  ...
  it("creates a worktree before starting the isolated sandbox", async () => {
    ...
    // After cleanup the worktree dir is gone, but the .red-castle/worktrees dir exists
    expect(existsSync(join(hostDir, ".red-castle", "worktrees"))).toBe(true);
  });
  ...
  it("provides hostWorktreePath in SandboxInfo", async () => {
    ...
    expect(receivedInfo!.hostWorktreePath).toContain(
      join(hostDir, ".red-castle", "worktrees"),
    );
  });
