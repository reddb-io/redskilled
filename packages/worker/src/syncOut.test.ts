...
describe("syncOut", () => {
  ...
  it("successful sync-out leaves no patch artifacts in .red-castle/patches", async () => {
    const hostDir = await mkdtemp(join(tmpdir(), "host-"));
    await initRepo(hostDir);
    await commitFile(hostDir, "initial.txt", "initial", "initial commit");
    ...
    try {
      ...
      const patchesDir = join(hostDir, ".red-castle", "patches");
  ...
  it("successful sync-out preserves tracked .red-castle files", async () => {
    const hostDir = await mkdtemp(join(tmpdir(), "host-"));
    await initRepo(hostDir);
    await commitFile(hostDir, "initial.txt", "initial", "initial commit");

    // Commit tracked files inside .red-castle, as a real repo would have
    await mkdir(join(hostDir, ".red-castle"), { recursive: true });
    await commitFile(
      hostDir,
      join(".red-castle", "prompt.md"),
      "prompt",
      "add .red-castle config",
    );
    await commitFile(
      hostDir,
      join(".red-castle", "Dockerfile"),
      "FROM node",
      "add Dockerfile",
    );
    ...
    try {
      ...
      // The whole .red-castle dir must not be deleted by cleanup
      expect(existsSync(join(hostDir, ".red-castle", "prompt.md"))).toBe(true);
      expect(existsSync(join(hostDir, ".red-castle", "Dockerfile"))).toBe(true);
      // But the temporary patches dir should still be cleaned up
      expect(existsSync(join(hostDir, ".red-castle", "patches"))).toBe(false);
    } finally {
      await handle.close();
    }
  });
