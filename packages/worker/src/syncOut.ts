 ...
 *    to `.red-castle/patches/<timestamp>/` before attempting to apply.
...
const createPatchDir = (
  ...
  Effect.tryPromise({
    try: async () => {
      ...
      const patchesRoot = join(hostRepoDir, ".red-castle", "patches");
 ...
 * 1. Save all artifacts to `.red-castle/patches/<timestamp>/`
...
export const syncOut = (
  ...
  Effect.gen(function* () {
    ...
    const relativePatchDir = join(".red-castle", "patches", basename(patchDir));
